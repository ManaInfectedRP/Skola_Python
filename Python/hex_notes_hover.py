...
class HexApp:
    ...
    # Lägg till i HexApp.__init__ efter self.canvas.bind(...)
        self.tooltip = None
        self.canvas.bind("<Motion>", self.on_mouse_move)
        self.canvas.bind("<Leave>", self.on_mouse_leave)

    def on_mouse_move(self, event):
        # Ta reda på vilket hex (q, r) musen är över
        q, r = self.pixel_to_axial(event.x, event.y)
        if (q, r) in self.hexes:
            data = self.data.get((q, r), {})
            ...
            # Visa tooltip nära muspekaren
            if self.tooltip is None:
                self.tooltip = tk.Label(self.canvas, text=tooltip_text, bg="lightyellow", 
            ...
            # Musen är inte över en hex — ta bort tooltip
            if self.tooltip:
